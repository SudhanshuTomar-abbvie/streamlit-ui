...
def save_message(message, update=False):
    ...
    if history["conversations"][conversation_id]["title"] == "New Conversation":
...
def sync_conversation_to_history():
    """
    Sync the current conversation from session state to history file
    This ensures that all messages in the current conversation are saved
    """
    if not hasattr(st.session_state, 'current_conversation_id'):
        return
    ...
    history["conversations"][conversation_id] = {
        "title": st.session_state.current_conversation_title,
        "created_at": history["conversations"].get(conversation_id, {}).get("created_at", datetime.now().isoformat()),
        "last_message_at": datetime.now().isoformat(),
