...
from config.settings import DOCUMENT_PROCESS_API_ENDPOINT, DOCUMENT_QUERY_API_ENDPOINT
...
def query_document(query, document_text):
    ...
    try:
        ...
        try:
            ...
            # Ensure we have an answer field, even if empty
            if "answer" not in result:
                result["answer"] = "Received response from API but no answer field was found."
            return result
        except ValueError:
            # If response is not valid JSON
            raise Exception(f"API did not return valid JSON: {response.text[:100]}...")
...
def document_chat_app():
    ...
    with st.sidebar:
        ...
        if st.button("Test API Connection"):
            try:
                # Simple test call to API
                test_payload = {"query": "test", "context": "This is a test."}
                response = requests.post(
                    DOCUMENT_QUERY_API_ENDPOINT,
                    json=test_payload,
                    headers={"Content-Type": "application/json"}
