...
def stream_response(response_text: str, placeholder):
    """
    Stream response with proper formatting preserved during streaming
    Uses smart token-aware streaming for better markdown handling
    ...
    # Split text into meaningful chunks that preserve markdown formatting
    # This pattern captures markdown elements as complete units
    pattern = r'(\*\*[^*]+\*\*|\*[^*]+\*|`[^`]+`|#{1,6}\s+[^\n]+|```[\s\S]*?```|\n+|\S+\s*)'
    tokens = re.findall(pattern, response_text)
    
    # If regex fails, fall back to simple word splitting
    if not tokens:
    ...
    for i, token in enumerate(tokens):
        ...
        # Vary the delay based on token type for better UX
        if token.startswith('#'):  # Headers
            time.sleep(0.1)
        elif token.startswith('**') or token.startswith('*'):  # Bold/italic
            time.sleep(0.05)
        elif token.startswith('```'):  # Code blocks
            time.sleep(0.15)
        elif '\n' in token:  # Line breaks
            time.sleep(0.08)
        elif token.strip().endswith(('.', '!', '?')):  # End of sentences
            time.sleep(0.1)
        else:  # Regular text
            time.sleep(0.03)
    
    # Final update to ensure complete formatting
    placeholder.markdown(displayed_text, unsafe_allow_html=True)
