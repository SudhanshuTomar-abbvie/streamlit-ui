...
def handle_message(message: str, is_document_query: bool = False) -> bool:
    ...
    # Handle the response
    if response["success"]:
        # Format the response with context in an expander if available
        response_content = response["message"]
        
        # Check if there's context to be put in an expander
        if isinstance(response_content, dict) and "answer" in response_content and "context" in response_content:
            formatted_content = f"{response_content['answer']}\n\n<details>\n<summary>Retrieved Context</summary>\n\n{response_content['context']}\n</details>"
        else:
