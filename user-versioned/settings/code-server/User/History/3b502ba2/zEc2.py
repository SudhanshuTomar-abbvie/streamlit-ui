...
if st.session_state.current_page == "home":
    ...
    st.image("avalogo.svg")
    # st.markdown(f'<div class="welcome-logo">{img_content}</div>', unsafe_allow_html=True)
    st.markdown('<h1 style="text-align:center; class="welcome-title">Welcome to MLOPS Chatbot</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align:center; class="welcome-subtitle">Patient Touchpoint Knowledge Agent</p>', unsafe_allow_html=True)
    st.markdown('<p style="text-align:center;>A generative AI chatbot designed to provide insights about patient touchpoints and healthcare experiences.</p>', unsafe_allow_html=True)
    ...
    # Main flex container for input + button
    # st.markdown('<div class="input-container">', unsafe_allow_html=True)

    # user_input = st.text_input("Ask any question...", key="home_input", label_visibility="collapsed")
...
# Chat page
    ...
    with chat_container:
        ...
        if not st.session_state.messages:
            # Initial welcome message with logo instead of PA
            st.markdown(
                f'''
