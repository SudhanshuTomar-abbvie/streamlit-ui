...
from utils.session_state import init_session_state, ensure_conversation_sync
...
def main():
    ...
    # Ensure conversation is synced (for existing conversations)
    if st.session_state.chat_history:
        ensure_conversation_sync()
    
    # Load custom CSS
    try:
        local_css("style.css")
    except:
        pass  # Continue if CSS file doesn't exist
    
    # Start the health check background thread
    try:
        start_health_check_thread()
    except:
        pass  # Continue if health check fails
