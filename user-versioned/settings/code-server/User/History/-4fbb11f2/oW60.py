...
def document_chat_app():
    st.markdown("## 📄 Document Chat Assistant", unsafe_allow_html=True)
    ...
    # -------------------------------
    # Sidebar – Upload & API Test
    # -------------------------------
    with st.sidebar:
        st.markdown("### 📤 Upload Document")
        uploaded_file = st.file_uploader("Choose a document", type=["pdf", "docx", "txt", "csv", "xlsx", "pptx", "html", "md"])
        ...
        if not st.session_state.document_text:
            ...
            with st.chat_message("assistant"):
                with st.spinner("💭 Thinking..."):
                    try:
                        response = query_document(query, st.session_state.document_text)
                        answer = response.get("answer", "🤷‍♂️ I couldn't find an answer.")
                        st.write(answer)
                        st.session_state.messages.append({"role": "assistant", "content": answer})
