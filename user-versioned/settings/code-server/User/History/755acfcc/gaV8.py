...
def generate_chat_response(user_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                response_data = response.json()
                # If backend (for whatever reason) returned a list, unpack it here:
                if isinstance(response_data, list):
                    # Assume [answer_str, context_str]
                    answer = response_data[0]
                    context = response_data[1] if len(response_data) > 1 else ""
                else:
