...
def generate_chat_response(user_query: str) -> Dict[str, Any]:
    ...
    Returns:
        API response containing the generated answer and context
    """
    import dataiku
    ...
    try:
        ...
        # Make the request to the webapp endpoint
        ...
        if response.status_code == 200:
            try:
                # Try to parse the response as JSON to extract message and context
                response_data = json.loads(response.text)
                
                # Check if we have context information in the response
                result = {"success": True}
                
                if isinstance(response_data, dict):
                    # If it's a structured response with separate message and context
                    result["message"] = response_data.get("message", response.text)
                    if "context" in response_data:
                        result["context"] = response_data["context"]
                else:
                    # If it's just the message as a string
                    result["message"] = response.text
                
                return result
            except json.JSONDecodeError:
                # If not JSON, just return the text as the message
