...
def render_chat_page():
    ...
    # Check and clear temporary document notification if time has passed
    current_time = time.time()
    if (st.session_state.get("show_doc_notification", False) and 
        current_time - st.session_state.get("doc_notification_time", 0) > 2):
        # Time exceeded, clear the notification flag
        st.session_state.show_doc_notification = False
        st.rerun()
    ...
    with st.container():
        ...
        if st.session_state.doc_query_mode:
            if st.session_state.extracted_doc_text:
                ...
                with col2:
                    if st.button("❌", key="remove_doc_btn", help="Remove document"):
                        # Clear document data
                        st.session_state.extracted_doc_text = None
                        ...
                        # Update the documents in document service
                        # This will update the count of documents
                        from services.document_service import remove_document
                        if st.session_state.get("active_doc_name"):
                            remove_document(st.session_state.get("active_doc_name"))
                        
                        # Disable document query mode
                        st.session_state.doc_query_mode = False
        ...
        # Check if we need to show the temporary document notification
        current_time = time.time()
        if (st.session_state.get("show_doc_notification", False) and 
            current_time - st.session_state.get("doc_notification_time", 0) < 2):
            # Document notification is still within the 2-second window
            docs = get_uploaded_documents()
            if docs:
                st.info(f"📄 {len(docs)} document{'s' if len(docs)>1 else ''} uploaded")
        # No else clause here - we handle clearing the notification in the header section
    ...
    with cols[1]:
        ...
        # Only show document count notification when:
        # 1. Not in document query mode
        # 2. Not showing temporary upload notification
        # 3. Actually have documents uploaded
        docs = get_uploaded_documents()
        if docs and not st.session_state.doc_query_mode and not st.session_state.get("show_doc_notification", False):
            st.info(f"📄 {len(docs)} document{'s' if len(docs)>1 else ''} uploaded")
            ...
            with st.container():
                ...
                if uploaded_file and not st.session_state.get("upload_in_progress", False):
                    ...
                    if processed:
                        ...
                        # Set a flag to show notification temporarily with timestamp
