        ...
        if st.session_state.get("show_uploader", False):
            with st.container():
                ...
                if uploaded_file and not st.session_state.get("upload_in_progress", False):
                    ...
                    with st.spinner("📚 Indexing your document into the knowledge base…"):
                        # Simulate or replace with real backend call here
                        processed = process_uploaded_file(uploaded_file)  # This should do the API call & indexing

                        # Optional: simulate delay
                        time.sleep(3)

                    if processed:
                        st.toast(f"✅ Document '{processed['name']}' has been added to the knowledge bank.")
                        st.session_state.uploaded_doc_name = processed['name']
                        st.session_state.upload_in_progress = False
                        st.session_state.show_uploader = False
                        st.rerun()
                    else:
                        st.error("❌ Failed to process and index the document.")
                        st.session_state.upload_in_progress = False
                        st.rerun()
