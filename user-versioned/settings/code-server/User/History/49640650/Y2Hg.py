...
def render_chat_page():
    ...
    with st.container():
        ...
        if st.session_state.get("temp_user_input"):
            ...
            st.rerun()
        # Handle post-upload message trigger
        if st.session_state.get("document_just_uploaded"):
            uploaded_name = st.session_state.get("uploaded_doc_name", "your document")

            with st.chat_message("assistant"):
                with st.spinner("Indexing your document..."):
                    # Trigger any post-upload response you want
                    handle_message(f"Document '{uploaded_name}' has been uploaded.", doc_uploaded=True)
            
            # Reset upload trigger
            del st.session_state.document_just_uploaded
            del st.session_state.uploaded_doc_name
        ...
        if "show_uploader" in st.session_state and st.session_state.show_uploader:
            with st.container():
                ...
                if uploaded_file:
                    ...
                    if processed:
                        st.success(f"📄 Document **{processed['name']}** uploaded successfully.")
                        st.session_state.document_just_uploaded = True
                        st.session_state.uploaded_doc_name = processed["name"]
                        # Optional: Temporarily keep uploader visible for a second rerun
                        # Or close it here based on your desired UX
                        # st.session_state.show_uploader = False
                        st.rerun()
                    else:
