...
def _on_send():
    """
    Callback for Send button or Enter key:
...
def render_chat_page():
    ...
    col_input = st.columns([1])[0]  # Single column for input with pin inside

    with col_input:
        if "user_input_text" not in st.session_state:
            st.session_state.user_input_text = ""
        
        # Add custom JavaScript for handling Enter key press
        st.markdown("""
            <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                setTimeout(function() {
                    const inputField = document.querySelector('div[data-testid="stText"] input');
                    if (inputField) {
        ...
        with input_container:
            ...
            with cols[1]:
                ...
                # Hide the submit button with CSS instead of a style parameter
                st.markdown("""
                    <style>
                    /* Hide the submit button but keep it functional */
                    div[data-testid="stButton"] button#submit-btn {
                        visibility: hidden;
                        height: 0px;
                        padding: 0px;
                        min-height: 0px;
                ...
                # Button without style parameter
                st.button("Submit", key="submit-btn", on_click=_on_send)
